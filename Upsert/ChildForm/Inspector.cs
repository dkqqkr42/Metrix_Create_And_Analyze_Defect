...
namespace Upsert
{
    public partial class Inspector : MetroForm, IChildForm
    {
        ...
        public string StoredProcedureName() => "INSPECTOR_UPSERT";
        ...
        public int SelectItem()
        {
            ...
            try
            {
                ...
                OracleCommand cmd = new OracleCommand
                {
                    ...
                    CommandText = "select trim(sa_sabun), trim(plant_code), trim(team_code), qc_code, trim(del_flag), insert_date, insert_user, update_date, update_user from tbl_inspector where trim(sa_sabun) like '%' || :SeachVal || '%' and DEL_FLAG = 'A'"
                };
                if (txt_SeachVal.Text.Equals("") && ckb_DelFlag.Checked)
                    cmd.CommandText = "select trim(sa_sabun), trim(plant_code), trim(team_code), qc_code, trim(del_flag), insert_date, insert_user, update_date, update_user from tbl_inspector";

                else if (txt_SeachVal.Text.Equals("") && !ckb_DelFlag.Checked)
                    cmd.CommandText = "select trim(sa_sabun), trim(plant_code), trim(team_code), qc_code, trim(del_flag), insert_date, insert_user, update_date, update_user from tbl_inspector where DEL_FLAG = 'A'";

                else if (!txt_SeachVal.Text.Equals("") && ckb_DelFlag.Checked)
                    cmd.CommandText = "select trim(sa_sabun), trim(plant_code), trim(team_code), qc_code, trim(del_flag), insert_date, insert_user, update_date, update_user from tbl_inspector where trim(sa_sabun) like '%' || :SeachVal || '%'";
        ...
        private void grd_Data_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            txt_SA_SABUN.Text = grd_Result.Rows[rowIndex].Cells[1].Value.ToString();
            txt_PLANT_CODE.Text = grd_Result.Rows[rowIndex].Cells[2].Value.ToString();
            txt_TEAM_CODE.Text = grd_Result.Rows[rowIndex].Cells[3].Value.ToString();
            txt_QC_CODE.Text = grd_Result.Rows[rowIndex].Cells[4].Value.ToString();
        ...
        public bool SaveItem()
        {
            ...
            try
            {
                ...
                if (txt_PLANT_CODE.Text.Equals("") || txt_TEAM_CODE.Text.Equals(""))
                ...
                cmd.Parameters.Add("IN_SA_SABUN", txt_SA_SABUN.Text);
                cmd.Parameters.Add("IN_PLANT_CODE", txt_PLANT_CODE.Text);
                cmd.Parameters.Add("IN_TEAM_CODE", txt_TEAM_CODE.Text);
                cmd.Parameters.Add("IN_QC_CODE", txt_QC_CODE.Text);
        ...
        public bool DeleteItem()
        {
            ...
            try
            {
                ...
                OracleCommand cmd = new OracleCommand
                {
                    CommandType = CommandType.Text,
                    Connection = connection,
                    CommandText = "update tbl_inspector set DEL_FLAG = 'D', update_date = SYSDATE where trim(sa_sabun) = :DeleteVal"
                };
