...
namespace Upsert
{
    public partial class DEPT : MetroForm, IChildForm
    {
        ...
        public string StoredProcedureName() => "DEPT_UPSERT";
        ...
        public int SelectItem()
        {
            ...
            try
            {
                ...
                OracleCommand cmd = new OracleCommand
                {
                    ...
                    CommandText = "select trim(dept_cd), dept_name, trim(team_code), trim(del_flag), insert_user, insert_date, update_user, update_date, trim(roll_flag) from tbl_dept where trim(DEPT_CD) like '%' || :SeachVal || '%' and DEL_FLAG = 'A'"
                };
                if (txt_SeachVal.Text.Equals("") && ckb_DelFlag.Checked)
                    cmd.CommandText = "select trim(dept_cd), dept_name, trim(team_code), trim(del_flag), insert_user, insert_date, update_user, update_date, trim(roll_flag) from tbl_dept";

                else if (txt_SeachVal.Text.Equals("") && !ckb_DelFlag.Checked)
                    cmd.CommandText = "select trim(dept_cd), dept_name, trim(team_code), trim(del_flag), insert_user, insert_date, update_user, update_date, trim(roll_flag) from tbl_dept where DEL_FLAG = 'A'";

                else if (!txt_SeachVal.Text.Equals("") && ckb_DelFlag.Checked)
                    cmd.CommandText = "select trim(dept_cd), dept_name, trim(team_code), trim(del_flag), insert_user, insert_date, update_user, update_date, trim(roll_flag) from tbl_dept where trim(DEPT_CD) like '%' || :SeachVal || '%'";
        ...
        private void grd_Data_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            txt_DEPT_CD.Text = grd_Result.Rows[rowIndex].Cells[1].Value.ToString();
            txt_DEPT_NAME.Text = grd_Result.Rows[rowIndex].Cells[2].Value.ToString();
        ...
        public bool SaveItem()
        {
            ...
            try
            {
                ...
                if (txt_DEPT_NAME.Text.Equals("") || txt_TEAM_CODE.Text.Equals(""))
                ...
                cmd.Parameters.Add("IN_DEPT_CD", txt_DEPT_CD.Text);
                cmd.Parameters.Add("IN_DEPT_NAME", txt_DEPT_NAME.Text);
                ...
                cmd.Parameters.Add("IN_ROLL_FLAG", txt_ROLL_FLAG.Text);
        ...
        public bool DeleteItem()
        {
            ...
            try
            {
                ...
                OracleCommand cmd = new OracleCommand
                {
                    CommandType = CommandType.Text,
                    Connection = connection,
                    CommandText = "update tbl_dept set DEL_FLAG = 'D', update_date = SYSDATE where trim(DEPT_CD) = :DeleteVal"
                };
