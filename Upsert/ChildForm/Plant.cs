...
namespace Upsert
{
    public partial class Plant : MetroForm, IChildForm
    {
        ...
        public string StoredProcedureName() => "PLANT_UPSERT";
        ...
        public int SelectItem()
        {
            ...
            try
            {
                ...
                OracleCommand cmd = new OracleCommand
                {
                    ...
                    CommandText = "select trim(plant_code), plant_name, trim(del_flag), insert_date, insert_user, update_date, update_user from tbl_plant where trim(plant_code) like '%' || :SeachVal || '%' and DEL_FLAG = 'A'"
                };
                if (txt_SeachVal.Text.Equals("") && ckb_DelFlag.Checked)
                    cmd.CommandText = "select trim(plant_code), plant_name, trim(del_flag), insert_date, insert_user, update_date, update_user from tbl_plant";

                else if (txt_SeachVal.Text.Equals("") && !ckb_DelFlag.Checked)
                    cmd.CommandText = "select trim(plant_code), plant_name, trim(del_flag), insert_date, insert_user, update_date, update_user from tbl_plant where DEL_FLAG = 'A'";

                else if (!txt_SeachVal.Text.Equals("") && ckb_DelFlag.Checked)
                    cmd.CommandText = "select trim(plant_code), plant_name, trim(del_flag), insert_date, insert_user, update_date, update_user from tbl_plant where trim(plant_code) like '%' || :SeachVal || '%'";
        ...
        private void grd_Data_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            txt_PLANT_NAME.Text = grd_Result.Rows[rowIndex].Cells[2].Value.ToString();
        ...
        public bool SaveItem()
        {
            ...
            try
            {
                ...
                if (txt_PLANT_NAME.Text.Equals("") || txt_DEL_FLAG.Text.Equals(""))
                ...
                cmd.Parameters.Add("IN_PLANT_NAME", txt_PLANT_NAME.Text);
        ...
        public bool DeleteItem()
        {
            ...
            try
            {
                ...
                OracleCommand cmd = new OracleCommand
                {
                    CommandType = CommandType.Text,
                    Connection = connection,
                    CommandText = "update tbl_plant set DEL_FLAG = 'D', update_date = SYSDATE where trim(plant_code) = :DeleteVal"
                };
