...
namespace Upsert
{
    public partial class DownTime : MetroForm, IChildForm
    {
        ...
        public string StoredProcedureName() => "DOWNTIME_UPSERT";
        ...
        public int SelectItem()
        {
            ...
            try
            {
                ...
                OracleCommand cmd = new OracleCommand
                {
                    ...
                    CommandText = "select trim(PLANT_CODE), trim(WC_CODE), trim(FACTOR_CODE), FACTOR_NAME, trim(DEL_FLAG), INSERT_DATE, INSERT_USER, UPDATE_DATE, UPDATE_USER from tbl_downtime where trim(WC_CODE) like '%' || :SeachVal || '%' and DEL_FLAG = 'A'"
                };
                if (txt_SeachVal.Text.Equals("") && ckb_DelFlag.Checked)
                    cmd.CommandText = "select trim(PLANT_CODE), trim(WC_CODE), trim(FACTOR_CODE), FACTOR_NAME, trim(DEL_FLAG), INSERT_DATE, INSERT_USER, UPDATE_DATE, UPDATE_USER from tbl_downtime";

                else if (txt_SeachVal.Text.Equals("") && !ckb_DelFlag.Checked)
                    cmd.CommandText = "select trim(PLANT_CODE), trim(WC_CODE), trim(FACTOR_CODE), FACTOR_NAME, trim(DEL_FLAG), INSERT_DATE, INSERT_USER, UPDATE_DATE, UPDATE_USER from tbl_downtime where DEL_FLAG = 'A'";

                else if (!txt_SeachVal.Text.Equals("") && ckb_DelFlag.Checked)
                    cmd.CommandText = "select trim(PLANT_CODE), trim(WC_CODE), trim(FACTOR_CODE), FACTOR_NAME, trim(DEL_FLAG), INSERT_DATE, INSERT_USER, UPDATE_DATE, UPDATE_USER from tbl_downtime where trim(WC_CODE) like '%' || :SeachVal || '%'";
        ...
        private void grd_Data_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            txt_WC_CODE.Text = grd_Result.Rows[rowIndex].Cells[2].Value.ToString();
            txt_FACTOR_CODE.Text = grd_Result.Rows[rowIndex].Cells[3].Value.ToString();
        ...
        public bool SaveItem()
        {
            ...
            try
            {
                ...
                if (txt_PLANT_CODE.Text.Equals("") || txt_WC_CODE.Text.Equals(""))
        ...
        public bool DeleteItem()
        {
            ...
            try
            {
                ...
                OracleCommand cmd = new OracleCommand
                {
                    CommandType = CommandType.Text,
                    Connection = connection,
                    CommandText = "update tbl_downtime set DEL_FLAG = 'D', update_date = SYSDATE where trim(WC_CODE) = :DeleteVal"
                };
